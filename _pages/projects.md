...
The second factorizing method I implemented is an algorithm that factors N = pq in subexponential time using the quadratic sieve method, based on the difference of squares factorization technique. For 100 bit primes p and q, this algorothm is essentially instant; for 250 bit primes, about 2.5 hours; for 500 bits, over 1000 years; beyond 500 bits, the time to factor is simply too long, as if 1000 years weren't already slow enough. Note that this timing was computed based on my own PC's machinery (standard Macbook Pro). 
